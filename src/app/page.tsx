  ...
  return<div style={{minHeight:'100dvh',paddingBottom:120,background:'#000'}}>
    ...
    {alerts.length>0&&tab==='home'&&<div style={{padding:'0 20px 16px'}}>
        <div ref={carouselRef} style={{display:'flex',overflowX:'auto',scrollSnapType:'x mandatory',WebkitOverflowScrolling:'touch',scrollbarWidth:'none',msOverflowStyle:'none',paddingBottom:8}} onScroll={(e)=>{const el=e.currentTarget;const cardW=el.offsetWidth;const idx=Math.round(el.scrollLeft/cardW);setInsightIdx(idx)}}>
          {alerts.map((a,i)=><div key={a.id} style={{width:'100%',minWidth:'100%',flexShrink:0,scrollSnapAlign:'start',padding:'16px 20px',borderRadius:16,fontSize:17,lineHeight:1.55,display:'flex',gap:12,alignItems:'flex-start',background:a.severity==='danger'?'var(--red-s)':a.severity==='warning'?'var(--orange-s)':a.severity==='success'?'var(--green-s)':'var(--blue-s)',border:`1px solid ${a.severity==='danger'?'rgba(255,69,58,0.15)':a.severity==='warning'?'rgba(255,159,10,0.15)':a.severity==='success'?'rgba(48,209,88,0.15)':'rgba(10,132,255,0.15)'}`}}>
    ...
    {tab==='home'&&<div className="tab-content" style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:20}}>
      ...
      {/* Stats */}
      <div className="fu s2" style={{display:'flex',gap:10}}>{[['Income',$(mInc),'var(--green)'],['Spent',$(mSp),'var(--orange)'],['Recurring',$(mRec),'var(--purple)']].map(([l,v,c],i)=><div key={i} className="gc" style={{flex:1,padding:'14px 10px',textAlign:'center'}}><div style={{fontSize:15,color:'var(--t3)',fontWeight:500,marginBottom:6}}>{l as string}</div><div className="mono" style={{fontSize:20,fontWeight:700,color:c as string}}>{v as string}</div></div>)}</div>
        ...
        <div className="gc">{(dateFrom||dateTo?txs.filter(t=>(!dateFrom||t.date>=dateFrom)&&(!dateTo||t.date<=dateTo)):txs).slice(0,20).map((tx,i)=><div key={tx.id} style={{position:'relative'}}>
          <div className="row" style={{...(i>0?{borderTop:'0.33px solid var(--sep)'}:{}),cursor:'pointer'}} onClick={()=>setTxMenu(txMenu===tx.id?null:tx.id)}>
            <div className="rb">
              ...
              <div className="rs">{tx.category} · {new Date(tx.date).toLocaleDateString('en-AU',{day:'numeric',month:'short'})}
                {tx.cost_centre_id&&ccs.find(c=>c.id===tx.cost_centre_id)&&<span style={{background:'var(--purple-s, rgba(139,92,246,0.15))',color:'var(--purple, #8b5cf6)',padding:'2px 8px',borderRadius:6,fontSize:12,fontWeight:600,marginLeft:6}}>{ccs.find(c=>c.id===tx.cost_centre_id)?.icon} {ccs.find(c=>c.id===tx.cost_centre_id)?.name}</span>}
                {tx.tags&&tx.tags.length>0&&tx.tags.map((tag:string,ti:number)=><span key={ti} style={{background:'var(--blue-s)',color:'var(--blue)',padding:'2px 8px',borderRadius:5,fontSize:12,fontWeight:600,marginLeft:4}}>{tag}</span>)}
              </div>
            </div>
          ...
          {txMenu===tx.id&&<div style={{padding:'12px 18px',background:'var(--card2)',borderTop:'0.33px solid var(--sep)'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
              {ccs.map(cc=><button key={cc.id} onClick={()=>assignCostCentre(tx.id,cc.id)} style={{padding:'8px 14px',borderRadius:10,border:tx.cost_centre_id===cc.id?'2px solid var(--orange)':'none',background:tx.cost_centre_id===cc.id?'var(--orange-s)':'var(--card)',color:'var(--t1)',fontSize:15,fontWeight:500,cursor:'pointer'}}>{cc.icon} {cc.name}</button>)}
    ...
    {tab==='budget'&&<div className="tab-content" style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:16}}>
      <div className="fu" style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}><div><h2 style={{fontSize:42,fontWeight:800,letterSpacing:-0.7}}>Budgets</h2><div style={{fontSize:16,color:'var(--t3)',marginTop:4}}>{$(mSp)} of {$(cats.reduce((s,c)=>s+Number(c.monthly_limit),0))} spent</div></div><button onClick={()=>newRow('budget_categories',{name:'',icon:'📁',color:'#ff9f0a',monthly_limit:0})} style={{padding:'8px 16px',borderRadius:10,border:'none',background:'var(--orange)',color:'#000',fontSize:14,fontWeight:600,cursor:'pointer'}}>+ Add</button></div>
    ...
    {tab==='subs'&&<div className="tab-content" style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      {flagged.length>0&&<><div className="sh" style={{color:'var(--red)'}}>⚠ Review</div><div className="gc fu s2">{flagged.map((s,i)=><div key={s.id} style={{padding:'16px 18px',...(i>0?{borderTop:'0.33px solid var(--sep)'}:{})}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:18,fontWeight:600}}>{s.name}</span><div style={{display:'flex',alignItems:'center',gap:8}}><span className="mono" style={{fontSize:15,fontWeight:600}}>{$$(Number(s.amount))}</span><span className="pill pill-r">{s.status==='duplicate'?'Duplicate':'Review'}</span></div></div>{s.notes&&<div style={{fontSize:14,color:'var(--t3)',marginTop:8}}>{s.notes}</div>}<div style={{fontSize:12,color:'var(--t3)',marginTop:4}}>{s.owner==='ben'?'👨 Ben':s.owner==='sarah'?'👩 Sarah':'👨‍👩‍👧‍👦 Family'}</div></div>)}</div></>}
      ...
      <div className="gc fu s3">{recs.filter(r=>r.status==='active').map((s,i)=><div key={s.id} style={{padding:'14px 18px',...(i>0?{borderTop:'0.33px solid var(--sep)'}:{})}}><div style={{display:'flex',alignItems:'center',gap:12}}><div style={{flex:1}}><div style={{fontSize:18,fontWeight:600}}>{s.name}</div><div style={{fontSize:13,color:'var(--t3)',marginTop:2}}>{s.category} · {s.frequency} · <span style={{color:s.owner==='ben'?'var(--blue)':s.owner==='sarah'?'var(--pink, #ff375f)':'var(--green)'}}>{s.owner==='ben'?'👨 Ben':s.owner==='sarah'?'👩 Sarah':'👨‍👩‍👧‍👦 Family'}</span>{s.tags&&s.tags.length>0&&s.tags.map((tag:string,ti:number)=><span key={ti} style={{background:'var(--orange-s)',color:'var(--orange)',padding:'2px 8px',borderRadius:5,fontSize:12,fontWeight:600,marginLeft:4}}>{tag}</span>)}</div></div><span className="mono" style={{fontSize:15,fontWeight:600,marginRight:8}}>{$$(Number(s.amount))}</span>
      ...
      <div className="gc fu s4" style={{padding:24,textAlign:'center',cursor:'pointer'}} onClick={()=>fileRef.current?.click()}><span style={{fontSize:50}}>📸</span><div style={{fontSize:17,fontWeight:500,marginTop:10}}>Snap a Bill</div><div style={{fontSize:16,color:'var(--t3)',marginTop:4}}>Fella reads it automatically</div></div>
    ...
    {tab==='reports'&&<div className="tab-content" style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:16,paddingBottom:20}}>
      <div className="fu"><h2 style={{fontSize:42,fontWeight:800,letterSpacing:-0.7}}>Reports</h2><div style={{fontSize:16,color:'var(--t3)',marginTop:4}}>View & export your financial data</div></div>
        ...
        return<>
          ...
          {reportTab==='summary'&&<div className="fu s3" style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div className="gc" style={{padding:18}}>
              ...
              {rByCat.slice(0,6).map((c,i)=><div key={c.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',...(i>0?{borderTop:'0.33px solid var(--sep)'}:{})}}><span style={{fontSize:16,fontWeight:500}}>{c.icon} {c.name}</span><span className="mono" style={{fontSize:15,fontWeight:600}}>{$(c.spent)}</span></div>)}
            </div>
            <div className="gc" style={{padding:18}}>
              <div style={{fontSize:16,fontWeight:600,marginBottom:12}}>Debts Snapshot</div>
              {debts.map((d,i)=>{const prog=pc(Number(d.original_amount)-Number(d.current_balance),Number(d.original_amount));return<div key={d.id} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',...(i>0?{borderTop:'0.33px solid var(--sep)'}:{})}}><span style={{fontSize:16,fontWeight:500}}>{d.name}</span><span className="mono" style={{fontSize:15}}><span style={{color:'var(--red)',fontWeight:600}}>{$$(Number(d.current_balance))}</span> <span style={{color:'var(--green)',fontSize:12}}>{prog}%</span></span></div>})}
            </div>
            <div className="gc" style={{padding:18}}>
              <div style={{fontSize:16,fontWeight:600,marginBottom:12}}>Goals Progress</div>
              {goals.map((g,i)=>{const prog=pc(Number(g.current_amount),Number(g.target_amount));return<div key={g.id} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',...(i>0?{borderTop:'0.33px solid var(--sep)'}:{})}}><span style={{fontSize:16,fontWeight:500}}>{g.icon} {g.name}</span><span className="mono" style={{fontSize:15}}>{$(Number(g.current_amount))} <span style={{color:'var(--t3)',fontSize:12}}>/ {$(Number(g.target_amount))}</span></span></div>})}
            </div>
          </div>}

          {/* SPENDING */}
          {reportTab==='spending'&&<div className="fu s3"><div className="gc">{rByCat.map((c,i)=>{const maxSp=Math.max(...rByCat.map(x=>x.spent),1);return<div key={c.id} style={{padding:'14px 18px',...(i>0?{borderTop:'0.33px solid var(--sep)'}:{})}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}><span style={{fontSize:16,fontWeight:500}}>{c.icon} {c.name}</span><span className="mono" style={{fontSize:14,fontWeight:600}}>{$(c.spent)}</span></div><div className="pbar" style={{height:8,borderRadius:4}}><div className="pfill" style={{width:`${(c.spent/maxSp)*100}%`,background:c.color,borderRadius:4}}/></div></div>})}</div></div>}
    ...
    {tab==='guide'&&<div className="tab-content" style={{padding:'0 20px 40px',display:'flex',flexDirection:'column',gap:24}}>
      ...
      <div id="guide-home" className="gc fu s2" style={{padding:0,overflow:'hidden'}}>
        ...
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
          ...
          ].map((item,i)=><div key={i} style={{paddingLeft:20,borderLeft:'3px solid var(--orange)'}}><div style={{fontSize:18,fontWeight:700}}>{item.t}</div><div style={{fontSize:16,color:'var(--t3)',marginTop:4,lineHeight:1.55}}>{item.d}</div></div>)}
        </div>
      </div>

      {/* BUDGET */}
      <div id="guide-budget" className="gc fu s3" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>📈 Budget</div><button onClick={()=>setTab('budget')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Track spending against monthly limits by category.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-debts" className="gc fu s4" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>🏦 Debts</div><button onClick={()=>setTab('debts')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Track and pay down everything you owe.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-fella" className="gc fu s5" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>💬 Fella</div><button onClick={()=>setTab('fella')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Your AI finance assistant. Named after Rockefeller.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-subs" className="gc fu s6" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>🔄 Subs & Bills</div><button onClick={()=>setTab('subs')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>All recurring costs and incoming bills in one place.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-goals" className="gc fu s7" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>🏖️ Goals</div><button onClick={()=>setTab('goals')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Set savings targets and track progress.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-kids" className="gc fu s8" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>👶 Cost Centres</div><button onClick={()=>setTab('kids')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Track spending per child, pet, or custom category.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-reports" className="gc fu s9" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>📊 Reports</div><button onClick={()=>setTab('reports')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Generate reports and export your data.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-settings" className="gc fu s10" style={{padding:0,overflow:'hidden'}}>
        <div style={{padding:'24px 24px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:28,fontWeight:800,letterSpacing:-0.3}}>⚙️ Settings</div><button onClick={()=>setTab('settings')} style={{padding:'10px 20px',borderRadius:12,border:'none',background:'var(--orange)',color:'#000',fontSize:15,fontWeight:600,cursor:'pointer'}}>Open →</button></div>
        <div style={{padding:'0 24px 8px',fontSize:17,color:'var(--t3)',lineHeight:1.6}}>Manage your data and connections.</div>
        <div style={{padding:'8px 24px 24px',display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div id="guide-soon" className="gc fu" style={{padding:24}}>
        ...
        ].map((item,i)=><div key={i} style={{paddingLeft:20,borderLeft:'3px solid var(--teal, #64d2ff)',marginBottom:16}}><div style={{fontSize:18,fontWeight:700}}>{item.t}</div><div style={{fontSize:16,color:'var(--t3)',marginTop:4,lineHeight:1.55}}>{item.d}</div></div>)}
      </div>
      ...
      <div className="gc fu" style={{padding:24,background:'var(--orange-s)'}}>
        ...
        <div style={{fontSize:17,color:'var(--t2)',lineHeight:1.7}}>
